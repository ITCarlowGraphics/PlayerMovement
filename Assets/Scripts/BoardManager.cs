...
public class BoardManager : MonoBehaviour
{
    ...
    public void Move(int player, int amount)
    {
        ...
        if (pm.space < 1 || pm.space > totalSpaces)
        {
        ...
        int currentIndex = pm.space - 1; // Convert to 0-based index
        int newIndex = currentIndex + amount; // Add the movement amount
        ...
        {
            newIndex %= totalSpaces; // Handle wrapping only if above 0
        }
        ...
        pm.space = newIndex + 1; // Convert back to 1-based index
    }
